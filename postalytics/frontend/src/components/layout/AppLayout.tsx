...
import {
  ...
  UserCircle, LogOut, ChevronLeft, Menu, Shield, BadgeCheck
} from "lucide-react";
...
export default function AppLayout() {
  ...
  return (
    <div className="flex h-screen bg-surface overflow-hidden">
      ...
      >
        {/* Header sidebar */}
        <div className={clsx("flex items-center gap-3 px-4 py-5 border-b border-navy-700", collapsed && "justify-center")}>
          ...
          {!collapsed && (
            <span className="font-display font-bold text-lg leading-none">PostalBI</span>
          )}
          <button
            onClick={() => setCollapsed(!collapsed)}
            className={clsx("ml-auto text-navy-700 hover:text-white transition-colors", collapsed && "ml-0 mt-0")}
          >
        ...
        <div className={clsx("border-t border-navy-700 p-3", collapsed && "flex justify-center")}>
          {collapsed ? (
            ...
            <div className="space-y-2">
              ...
              <span className={clsx("role-badge w-full justify-center", roleInfo.color)}>
                {roleInfo.label}
              </span>
              <button
                onClick={logout}
                className="flex items-center gap-2 text-xs text-gray-400 hover:text-white w-full px-2 py-1 rounded hover:bg-navy-700 transition-colors"
              >
